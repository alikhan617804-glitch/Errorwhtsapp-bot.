        ...
        if (config.AUTO_STATUS_REPLY && !m.key.fromMe) {
            ...
            try {
                await sock.sendMessage(
                    replyTo,
                    {
                        text: config.AUTO_STATUS_MSG || 'Seen by ERROR 💖',
                        contextInfo: {
                            stanzaId:      statusId,
                            participant:   replyTo,
                            quotedMessage: inner,
        ...
        if (config.Status_Saver === 'true' && typeof saveMedia === 'function') {
            try {
                ...
                if (config.STATUS_REPLY === 'true') {
                    const replyMsg = config.STATUS_MSG || 'ERROR 💖 SUCCESSFULLY VIEWED YOUR STATUS';
...
function getAutoStatusSettings() {
    ...
    return {
        ...
        statusReplyText:  config.AUTO_STATUS_MSG      || 'Seen by ERROR 💖',
        ...
        statusSaverMsg:   config.STATUS_MSG           || 'ERROR 💖 SUCCESSFULLY VIEWED YOUR STATUS',
    };
}
