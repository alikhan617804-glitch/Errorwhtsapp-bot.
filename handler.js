...
async function handleMessages(sock, message) {
    try {
            ...
            if (afkPlugin?.isAfk()) {
                ...
                await safeSend(sock, jid, {
                    text: [
                        ...
                        ``,
                        `_${th.footer || th.botName || 'ERROR'}_`
                    ].join('\n'),
                }, { quoted: message });
                return;
            }
        ...
        if (usedPrefix === null) {
            // Allow "ERROR" or "agent" to trigger the AI assistant without any prefix
            if (/^(ERROR|agent)\b/i.test(text.trim())) {
                usedPrefix  = '';
                commandText = text.trim();
            } else {
        ...
        if (isLid) {
            ...
            // 2. Fall back to the global LID→phone cache populated by ERROR.js
