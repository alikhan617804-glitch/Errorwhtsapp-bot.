// ERROR.js — Updated with fixes for group functionality and error handling
...
const tempDir = path.join(os.tmpdir(), 'error-bot-cache');
...
async function loadSession() {
    try {
        ...
        if (sid && hasTilde) {
            // SESSION_ID is provided in ERROR~<b64> format.
...
function generateFancyBio() {
    ...
    const bios = [
        ...
        `⚡ ERROR Active ✦ ${timeStr} ✦ ${dateStr} ✦`,
        `💫 ${config.BOT_NAME} Operational ✦ ${dateStr} ✦`,
        `🚀 ERROR Live ✦ ${dateStr} ✦ ${timeStr} ✦`,
...
async function sendWelcomeMessage(sock) {
    ...
    const caption = [
        `╔══════════════════════════╗`,
        `║  ⚡  *${(config.BOT_NAME || 'ERROR').toUpperCase()}*  ⚡  ║`,
    ...
    const iconPath = path.join(__dirname, 'assets', 'ERROR-WhatsApp-Bot-Promotional-Poster.png');
...
async function connectToWhatsApp() {
    ...
    global.errorWhatsAppSocket = sock;
    ...
    sock.ev.on('connection.update', async update => {
        ...
        if (qr) {
            latestPairingQR = qr;
            logMessage('INFO', '📱 QR code ready — open /pair and scan it in WhatsApp > Linked Devices');
        }
        ...
        if (connection === 'close') {
            ...
            // ── Auto-follow ERROR 💀 updates newsletter on startup ────────────
            setTimeout(async () => {
...
// ── ERROR 💀 Pairing Center state ──────────────────────────────────────────
// QR is kept only in memory and expires when WhatsApp rotates it.
...
<title>ERROR 💀 — WhatsApp Pairing Center</title>
<style>
body{margin:0;background:#090909;color:#fff;font-family:Arial,sans-serif;display:flex;justify-content:center;min-height:100vh}
.card{width:min(92vw,560px);margin:35px 0;padding:28px;border:1px solid #333;border-radius:20px;background:#111;box-shadow:0 10px 40px #000}
h1{margin:0 0 8px;font-size:30px}.muted{color:#aaa}.qr{display:flex;justify-content:center;margin:22px 0;min-height:280px;align-items:center}
img{max-width:280px;border-radius:12px;background:#fff;padding:10px}.row{display:flex;gap:10px}.row>*{flex:1}
input,button{box-sizing:border-box;width:100%;padding:14px;border-radius:10px;border:1px solid #444;background:#181818;color:#fff;font-size:16px}
button{cursor:pointer;background:#fff;color:#000;font-weight:700}
.code{font-size:30px;letter-spacing:5px;text-align:center;padding:15px;margin-top:15px;background:#181818;border-radius:12px}
.status{padding:12px;border-radius:10px;background:#181818;margin-top:15px}.small{font-size:13px;color:#999;margin-top:14px}
</style>
</head>
<body><main class="card">
<h1>💀 ERROR WhatsApp Bot</h1>
<div class="muted">Secure Pairing Center • QR + Phone Pairing</div>
<div class="qr" id="qr">Loading QR…</div>
<div class="row">
<input id="phone" inputmode="numeric" placeholder="Phone number with country code">
<button onclick="pair()">Get Pairing Code</button>
</div>
<div id="code"></div>
<div class="status" id="status">Waiting for WhatsApp session…</div>
<div class="small">Owner: 03240993066 • Official updates channel is configured in the bot.</div>
<script>
async function refresh(){
  try{
    const r=await fetch('/api/pair/status'); const d=await r.json();
    const q=document.getElementById('qr');
    if(d.connected){q.innerHTML='<div>✅ Bot is already connected.</div>';document.getElementById('status').textContent='Connected to WhatsApp';return;}
    if(d.qr){q.innerHTML='<img alt="WhatsApp QR" src="https://api.qrserver.com/v1/create-qr-code/?size=280x280&data='+encodeURIComponent(d.qr)+'">';}
    else q.innerHTML='<div>⏳ QR not ready yet.<br>Keep this page open.</div>';
    document.getElementById('status').textContent=d.busy?'Generating pairing code…':'Ready';
  }catch(e){document.getElementById('status').textContent='Status unavailable';}
}
async function pair(){
  const phone=document.getElementById('phone').value.trim();
  if(!phone)return alert('Enter the WhatsApp number with country code.');
  document.getElementById('code').textContent='Generating…';
  try{
    const r=await fetch('/api/pair/code',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone})});
    const d=await r.json();
    if(!r.ok) throw new Error(d.error||'Failed');
    document.getElementById('code').innerHTML='<div class="code">'+d.code+'</div>';
    document.getElementById('status').textContent='Enter this code in WhatsApp → Linked Devices → Link with phone number.';
  }catch(e){document.getElementById('code').textContent='❌ '+e.message;}
}
...
app.get('/api/pair/status', (req, res) => {
    res.json({
        connected: !!global.errorWhatsAppSocket?.user,
        qr: latestPairingQR,
        busy: pairingBusy
    });
});
...
app.post('/api/pair/code', async (req, res) => {
    try {
        ...
        if (!phone || phone.length < 7 || phone.length > 15) {
            return res.status(400).json({ error: 'Invalid phone number. Use country code + number.' });
        }
        const sock = global.errorWhatsAppSocket;
        if (!sock || typeof sock.requestPairingCode !== 'function') {
            return res.status(503).json({ error: 'WhatsApp socket is not ready yet. Open /pair and try again in a few seconds.' });
        }
        if (sock.user) {
            return res.status(409).json({ error: 'This bot is already connected. Log out/clear the session before pairing another number.' });
        }
        pairingBusy = true;
        latestPairingQR = null;
        const code = await sock.requestPairingCode(phone);
        pairingBusy = false;
        return res.json({ code: String(code || '').match(/.{1,4}/g)?.join('-') || String(code || '') });
    } catch (e) {
        pairingBusy = false;
...
app.get('/', (req, res) => {
    const html = path.join(__dirname, 'smm', 'ERROR.html');
...
const GH_OWNER = 'ERROR';
const GH_REPO  = 'ERROR-WhatsApp-Bot';
...
async function ghFetch(endpoint) {
    ...
    const headers = { 'User-Agent': 'ERROR-WhatsApp-Bot', 'Accept': 'application/vnd.github+json' };
...
app.get('/api/developer', async (req, res) => {
    ...
    if (result.ok) return res.json(result.data);
    res.json({ _error: result.reason, login: GH_OWNER, name: 'ERROR 💀', bio: '', followers: null, public_repos: null, avatar_url: '' });
});
...
(async () => {
    try {
        ...
        console.log('║     ERROR 💀  •  v2.0           ║');
        console.log('╚══════════════════════════════════════════╝');
        console.log('\x1b[0m');
        logMessage('INFO', 'Booting ERROR WhatsApp Bot...');
