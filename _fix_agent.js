...
let src = fs.readFileSync('plugins/ERROR-agent.js', 'utf8');
        ...
        return await Promise.any([
            tryOne(async () => {
                const res = await axios.post('https://ch.at/api/chat',
                    { message: fullPrompt },
                    { headers: { 'Content-Type': 'application/json', 'User-Agent': 'ERRORMD-Bot/2.0' }, timeout: 10000 }
                );
            ...
            tryOne(async () => {
                const res = await axios.get('https://api.popcat.xyz/chatbot?msg=' + encodeURIComponent(query) + '&owner=' + encodeURIComponent(config.OWNER_NAME || 'ERROR') + '&botname=ERROR', { timeout: 8000 });
...
const newElse = `        } else {
            ...
            if (smart) {
                ...
                const systemPrompt =
                    'You are ERROR, an intelligent WhatsApp bot assistant. ' +
                    'Owner: ' + (config.OWNER_NAME || 'ERROR') + '. Bot: ' + (config.BOT_NAME || 'ERROR') + '. ' +
                    ...
                    try {
                        ...
                        if (apiKey) {
                            ...
                            const geminiHistory = [
                                ...
                                { role: 'model', parts: [{ text: 'Got it! I am ERROR, ready to help.' }] },
                                ...mem.slice(0, -1).map(m => ({
...
fs.writeFileSync('plugins/ERROR-agent.js', src, 'utf8');
